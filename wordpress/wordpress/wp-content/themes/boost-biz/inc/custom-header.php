 ...
 *
 * @uses boost_biz_header_style()
 */
function boost_biz_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'boost_biz_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'boost_biz_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'boost-biz' ),
		),
	) );
}
add_action( 'after_setup_theme', 'boost_biz_custom_header_setup' );

if ( ! function_exists( 'boost_biz_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see boost_biz_custom_header_setup().
	 */
	function boost_biz_header_style() {
		...
		wp_add_inline_style( 'boost-biz-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'boost_biz_header_style', 10 );