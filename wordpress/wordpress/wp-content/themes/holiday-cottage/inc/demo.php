 ...
 * @since 1.0.0
 */
function holiday_cottage_ocdi_after_import() {
...
add_action( 'pt-ocdi/after_import', 'holiday_cottage_ocdi_after_import' );
 ...
 */
function holiday_cottage_additional_intro_text( $intro ) {
	$info = '';

	$info .= '<div class="ocdi__intro-text">';
	$info .= '<p><strong>' . esc_html__( 'NOTE: ', 'holiday-cottage' );
	$info .= esc_html__( 'Before importing, please make sure that Advanced Booking Calendar plugin is installed and activated. Rooms should be properly setup to display room sections in the front page.', 'holiday-cottage' );
	$info .= '</strong></p>';
	$info .= '</div>';
...
add_filter( 'pt-ocdi/plugin_intro_text', 'holiday_cottage_additional_intro_text' );
